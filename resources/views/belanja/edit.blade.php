        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                ...
                <p class="text-xs sm:text-sm text-slate-500">
                    Ubah informasi barang yang sudah kamu tambahkan.
                </p>
            </div>
        </div>
        ...
        <form action="{{ route('belanja.item.update', $item->id) }}" method="POST" class="space-y-5">
            ...
            <div>
                <label for="status" class="block text-sm font-semibold text-slate-800 mb-1.5">
                    Status <span class="text-slate-400">*</span>
                </label>
                <select
                    name="status"
                    id="status"
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-sm sm:text-base text-slate-900 shadow-sm focus:border-[#ed000c] focus:ring-2 focus:ring-[#ed000c]/20 focus:outline-none"
                >
                    <option value="Sudah Dibeli" {{ $item->status == 'Sudah Dibeli' ? 'selected' : '' }}>
                        Sudah Dibeli
                    </option>
                    <option value="Belum Dibeli" {{ $item->status == 'Belum Dibeli' ? 'selected' : '' }}>
                        Belum Dibeli
                    </option>
                </select>
            </div>
